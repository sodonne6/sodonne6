...
Element* createNewElement(char* name, Element* ht[]) {
    ...
    while (ht[current_slot] != NULL){ //while the current slot is taken, add 1 to collisions and move onto the next spot
        collisions++; 
        current_slot = (current_slot + 1) % ARRAY_SIZE; //keep going through each slot until you find a space each time its full add 1 to the slot

        if(current_slot == initial_slot){ // if the function goes through the whole hash table and returns to the first slot, the hash table is full and can't take anymore data
            printf("Hash Table is full\n"); //once it gets back to the original slot (lets say 5), once it gets to 15 it'll modulus it and realise its back at 5
...
Element* search (char* name,Element* ht[]){
    ...
    int current_location = (list_loc+1) % ARRAY_SIZE; //move on to the next slot
    int initial_location = list_loc; // save this as the original spot so once we get back there we know storage full
...
void insert(char* name, Element* ht[]){
 int list_loc =hash_function(name);
 if (ht[list_loc] == NULL){
    ht[list_loc]=createNewElement(name, ht); //if a space is empty call createNewElement function and park that data in there
 } else{
    collisions++;
 }
...
void printNumberOfOccurences(char* name, Element* ht[]){
    //TODO5
    Element* foundElement = search(name,ht); //search table for given name
    if(foundElement == NULL){ //name is not present
        printf("%s - 0 \n", name);
    } 
    else{
...
void addOrIncrement(char* name,Element* ht[]){
 Element* foundElement = search(name,ht); //search hash table for certain name   
if(foundElement != NULL){ //if name is found add to count 
    foundElement->count++;
}
else{
    createNewElement(name,ht); // if the name isnt found in the table call the createNewElement funct.
}
}
